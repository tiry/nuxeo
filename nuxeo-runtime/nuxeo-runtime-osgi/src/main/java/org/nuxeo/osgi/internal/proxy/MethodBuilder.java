...
public class MethodBuilder {
    ...
    interface MHTransformer {
        MethodHandle transform(MethodHandle mh)
    ...
    public interface Fun<T, R> {
         ...
         *             throws is a method is not visible.
         * @throws NoSuchFieldException
         *             throws if a field is not visible.
         * @throws IllegalAccessException
         *             throws if a type or a member of a type is not visible.
         */
     ...
     *
     * @param methodType
     *            the method type that the {@link #call(MethodHandle) resulting method handle}.
    ...
    public MethodBuilder boxAll() {
        ...
        return apply(methodType(sig.returnType(), Object[].class),
                mh -> transformer.transform(mh.asCollector(Object[].class, sig.parameterCount()).asType(sig)));
    }
     ...
     *            position of the inserted value in the parameters
     * @param type
    ...
    public <T> MethodBuilder insertAt(int parameterIndex, Class<T> type, T value) {
        MHTransformer transformer = this.transformer;
        return apply(sig.insertParameterTypes(parameterIndex, type),
                mh -> transformer.transform(insertArguments(mh, parameterIndex, value)));
    }
     ...
     * 
     * @param parameterIndex
     *            position of the dropped parameter
     * @return the current method builder
     */
    public MethodBuilder dropAt(int parameterIndex) {
        Class<?> type = sig.parameterType(parameterIndex);
        MHTransformer transformer = this.transformer;
        return apply(sig.dropParameterTypes(parameterIndex, parameterIndex + 1),
                mh -> transformer.transform(dropArguments(mh, parameterIndex, type)));
    }
     ...
     * 
     * @param methodType
     *            the expected type of the parameter and return type.
     * @return the current method builder
     */
     ...
     * 
     * @param function
     *            a function that specify the code to execute before the current code,
     ...
     * 
     * @param function
     *            a function that specify the code to execute after the current code,
     ...
     * @param function
     *            a function that specify the code to execute when an exception is thrown.
     *            The method handle returned by the function must have the same signature as the current code.
     * @return the current method builder
     */
    public MethodBuilder trap(Class<? extends Throwable> exceptionType,
            Fun<? super MethodBuilder, ? extends MethodHandle> function) {
        MethodType instrType = sig.insertParameterTypes(0, exceptionType);
        MHTransformer transformer = this.transformer;
        return apply(sig, mh -> transformer
                .transform(catchException(mh, exceptionType, function.apply(methodBuilder(instrType)))));
    }
    ...
    public MethodBuilder compose(Class<?> returnType, Fun<? super MethodBuilder, ? extends MethodHandle> function) {
        ...
        MHTransformer transformer = this.transformer;
        return apply(methodType(sig.returnType(), returnType),
                mh -> transformer.transform(filterReturnValue(function.apply(methodBuilder(instrType)), mh)));
    }

    public MethodBuilder filter(int argumentIndex, Class<?> returnType,
            Fun<? super MethodBuilder, ? extends MethodHandle> function) {
        ...
        return apply(sig.changeParameterType(argumentIndex, returnType),
                mh -> transformer
                        .transform(filterArguments(mh, argumentIndex, function.apply(methodBuilder(filterType)))));
    }

    public MethodBuilder filterLast(int argumentCount, Class<?> fromType, Class<?> toType,
            Fun<? super MethodBuilder, ? extends MethodHandle> function) {
     ...
     *             throws if a type or a member of a type is not visible.
     */
    public MethodHandle call(MethodHandle target)
            throws NoSuchMethodException, NoSuchFieldException, IllegalAccessException {
     ...
     *
     * @param lookup
     *            the lookup object used to find the @code method}
     * @param method
     *            the method called at the end of the transformation.
     ...
     *             throws if a type or a member of a type is not visible.
     */
    public MethodHandle unreflect(Lookup lookup, Method method)
            throws NoSuchMethodException, NoSuchFieldException, IllegalAccessException {
     ...
     *             throws if a type or a member of a type is not visible.
     */
    public MethodHandle callConstant(Object value)
            throws NoSuchMethodException, NoSuchFieldException, IllegalAccessException {
        return call(constant(sig.parameterType(0), value));
    }

    static class InliningCacheCallSite extends MutableCallSite {
        private static final int MAX_KIND_OF_TYPE = 8; // number of kind of type before considering the callsite as
                                                       // megamorphic
        ...
        static {
            ...
            try {
                CLASS_CHECK = lookup.findStatic(thisClass, "classCheck",
                        methodType(boolean.class, Object.class, Class.class));
                FALLBACK = lookup.findStatic(thisClass, "fallback",
                        methodType(MethodHandle.class, thisClass, Object.class));
        ...
        InliningCacheCallSite(MethodHandle endPoint) {
            ...
            setTarget(foldArguments(exactInvoker(type),
        ...
        private static MethodHandle fallback(InliningCacheCallSite callsite, Object receiver) throws Throwable {
            ...
            MethodHandle test = insertArguments(CLASS_CHECK, 1, receiverClass)
                    .asType(methodType(boolean.class, endPointType.parameterType(0)));
            MethodHandle target = endPoint.asType(endPointType.changeParameterType(0, receiverClass))
                    .asType(endPointType); // insert a cast to help the JIT ?
