...
import org.nuxeo.osgi.internal.nio.BundleWalker;
...
public abstract class ApplicationLoader {
    ...
    public void setScanForNestedJARs(boolean scanForNestedJARs) {
        osgi.setProperty(LoaderConstants.SCAN_FOR_NESTED_JARS,
                Boolean.toString(scanForNestedJARs));
    }

    public boolean getScanForNestedJARs() {
        return Boolean.valueOf(osgi.getProperty(LoaderConstants.SCAN_FOR_NESTED_JARS, "false")).booleanValue();
    }
     ...
     * @param bundles
     *            the list to fill with found bundles
     * @param jars
     *            the list to fill with found jars
     * @throws IOException
     */
    public void install(File root, List<Bundle> bundles) throws IOException {
        BundleInstaller callback = new BundleInstaller(bundles);
        BundleWalker visitor = new BundleWalker(osgi.getSystemBundle().getBundleContext(), callback, patterns);
        visitor.visit(root);
    }
