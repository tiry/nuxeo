...
public class ClassPath {
    ...
    public Bundle[] scan(List<File> files) {
        ...
        new BundleWalker(osgi.getSystemBundle().getBundleContext(), collector);
    ...
    public Bundle[] scan(List<File> files, String[] blacklist) {
        BundleCollector collector = new BundleCollector();
        new BundleWalker(osgi.getSystemBundle().getBundleContext(), collector, blacklist);
        return bundles = collector.bundles.toArray(new Bundle[collector.bundles.size()]);
    }
    ...
    public void restore(File file) throws IOException {
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    list.add(osgi.installBundle(f.toURI()));
                } catch (BundleException cause) {
                    throw new IOException("Cannot reload bundle file " + f, cause);
                }
            }
        } finally {
            if (reader != null) {
