 ...
 * as number of milliseconds since January 1, 1970, 00:00:00 GMT. The constraint
 * finally supports String having YYYY-MM-DD format.
 * </p>
 *
 * @since 7.1
 */
public class DateIntervalConstraint extends AbstractConstraint {
     ...
     * <p>
     * Bounds could be any {@link Date} or {@link Calendar}. Bounds also support
     * {@link Number} types whose long value is recognised as number of
     * milliseconds since January 1, 1970, 00:00:00 GMT. Bounds finally supports
     * String having YYYY-MM-DD format.
     * </p>
     * <p>
     ...
     */
    public DateIntervalConstraint(Object minDate, boolean includingMin,
            Object maxDate, boolean includingMax) {
        ...
        if (this.minTime != null && this.maxTime != null
            ...
            log.warn("lower bound (" + minDate
                    + ") is greater than upper bound (" + maxDate
                    + "). No dates could be valid.");
        }
        if ((this.minTime == null && minDate != null)
                || (this.maxTime == null && maxDate != null)) {
            log.warn("some bound was ignored due to invalid date format (supported format is "
                    + ConstraintUtils.DATE_FORMAT
                    + " (min = "
                    + minDate
                    + " - max = " + maxDate + ")");
        }
    ...
    public Description getDescription() {
        ...
        if (this.minTime != null) {
            ...
            params.put(PNAME_MIN_INC, this.includingMin);
        }
        if (this.maxTime != null) {
            params.put(PNAME_MAXIMUM, new Date(this.maxTime));
            params.put(PNAME_MAX_INC, this.includingMax);
        }
